(function (global) {
    ...
    coa_moreViewModel = kendo.data.ObservableObject.extend({
        ...
        cLink: function () {
            ...
            console.log('download iosURL :' + iosURL);
            console.log('download androidURL :' + androidURL);
            if (localStorage.getItem("OS") == "ios") {
                window.open('itms-services://?action=download-manifest&url=' + iosURL, '_blank', 'location=no,EnableViewPortScale=yes');
            } else {
                window.open(androidURL, '_system', 'location=no,EnableViewPortScale=yes');
            }
        },
