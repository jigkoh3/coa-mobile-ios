(function (global) {
    ...
    boxViewModel = kendo.data.ObservableObject.extend({
        ...
        setWarning: function (statusID, xhrText) {
            ...
            if (statusID != 0 || statusID != "0") {
                if (statusID == 1 || statusID == 200) {
                    console.log(xhrText.length);
                    if (xhrText.length > 500) {
                    ...
                    if (xhrText != "" && xhrText != null) {
                        if (JSON.parse(xhrText).RESPONSE_MSG != undefined) {
                            ...
                            if (JSON.parse(xhrText).RESPONSE_CODE == "3004" || JSON.parse(xhrText).RESPONSE_CODE == "3001") {
        ...
        setConfirmAlert: function (text, link) {
            var that = app.coa_boxService.viewModel;
            kendo.bind($("#div_confirmAlert"), that);
            that.set('cText', text);
            that.set('cLink', link);
            $("#div_confirmAlert").show();
        },
